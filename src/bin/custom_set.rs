...
impl<T: Hash + Eq + Copy> CustomSet<T> {
    pub fn new(input: &[T]) -> Self {
        Self {
            value: HashMap::from_iter(input.iter().cloned().zip(iter::repeat(()))),
        }
    }

    pub fn contains(&self, element: &T) -> bool {
        self.value.contains_key(element)
    }
    ...
    pub fn is_subset(&self, other: &Self) -> bool {
        self.value.keys().all(|k| other.contains(k))
    }
    ...
    pub fn is_disjoint(&self, other: &Self) -> bool {
        !self.value.keys().any(|k| other.contains(k))
    }
