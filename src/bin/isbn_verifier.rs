...
pub fn is_valid_isbn(isbn: &str) -> bool {
    isbn.chars()
        .try_fold((10, 0), |(mul, sum), c| match c {
            '0'..='9' => {
                println!("{sum}");
                Some((mul - 1, sum + mul * (c.to_digit(10).unwrap() as i32)))
            }
            'X' if mul == 1 => Some((mul - 1, sum + 10)),
            '-' => Some((mul, sum)),
            _ => None,
        })
        .is_some_and(|(mul, sum)| mul == 0 && sum != 0 && sum % 11 == 0)
}
