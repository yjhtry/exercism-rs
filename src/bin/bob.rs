pub fn reply(message: &str) -> &str {
    ...
    let is_upper = message.to_uppercase() == message;
    let has_alpha = message.chars().any(|c| c.is_ascii_alphabetic());
    let end_with_qs = message.ends_with("?");

    match (is_empty, is_upper, has_alpha, end_with_qs) {
